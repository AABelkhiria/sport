...
void sensor_service_thread(void* p1, void* p2, void* p3)
{
    ...
    while (1)
    {
        if (rc == 0)
        {
            rc = sensor_channel_get(gyro_sensor, SENSOR_CHAN_ACCEL_XYZ,
                accel);
        }
        if (rc == 0)
        {
            rc = sensor_channel_get(gyro_sensor, SENSOR_CHAN_GYRO_XYZ,
                gyro);
        }
        if (rc == 0)
        {
            rc = sensor_channel_get(gyro_sensor, SENSOR_CHAN_DIE_TEMP,
                &temperature);
        }
        if (rc == 0)
        {
...
K_THREAD_DEFINE(sensor_tid, CONFIG_SENSOR_SERVICE_STACK_SIZE, sensor_service_thread, NULL, NULL, NULL,
    CONFIG_SENSOR_SERVICE_PRIORITY, 0, 0);
